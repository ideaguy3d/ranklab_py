...
This keeps agent roles reusable across very different domains.
Tune `PROJECT_NAME` and the instructions text for each new subproject.
"""
...
PROJECT_NAME = "resume-customizer"
...
def build_task_checklist(goal: str, constraints: str = "", max_steps: int = 6) -> str:
    ...
    Returns JSON with goal, constraints, and ordered steps.
    """
...
def evaluate_options(options_json: str, criteria_json: str) -> str:
    """
    Compare options against weighted criteria.
    `options_json`: JSON array of option names.
    `criteria_json`: JSON array of objects:
      [{"name":"speed","weight":0.4,"scores":{"Option A":7,"Option B":9}}]
    Returns ranked JSON output.
    """
...
planner_agent = Agent(
    ...
    handoff_description=(
        "Clarifies the user's goal, gathers missing inputs, and creates a concise plan."
    ),
    instructions=f"""
...
analyst_agent = Agent(
    ...
    handoff_description=(
        "Performs structured analysis, comparisons, and tradeoff evaluation."
    ),
    instructions=f"""
...
writer_agent = Agent(
    ...
    handoff_description=(
        "Turns analysis into user-ready output: recommendations, drafts, or action steps."
    ),
    instructions=f"""
...
router_agent = Agent(
    ...
    instructions=f"""
You are the main router for the {PROJECT_NAME} app.

Routing policy:
- Start with yourself for simple questions and quick answers.
- Handoff to Task Planner Agent when goals are unclear or multi-step.
- Handoff to Domain Analyst Agent for deeper reasoning or comparisons.
- Handoff to Output Composer Agent when final packaging or rewrite quality is needed.

Global rules:
- Keep responses honest, concise, and useful.
- Ask clarifying questions only when necessary.
- If user intent changes, re-route to the most appropriate agent.
""",
