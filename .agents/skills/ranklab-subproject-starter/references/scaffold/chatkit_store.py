"""Simple in-memory ChatKit store for demo-stage RankLab subprojects."""
...
class InMemoryChatKitStore(Store[dict]):
    """Minimal in-memory implementation of ChatKit storage."""
    ...
    async def save_item(self, thread_id: str, item: ThreadItem, context: dict) -> None:
        for idx, existing in enumerate(self.items[thread_id]):
            if existing.id == item.id:
                self.items[thread_id][idx] = item
                return
        self.items[thread_id].append(item)
